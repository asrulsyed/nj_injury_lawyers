import { NavLink } from "react-router-dom";

const Blogs = () => {
  return (
    <div className="flex justify-center">
      <div className="flex w-[80%] max-w-[800px] flex-col gap-5">
        <NavLink to="/blogs/1">
          <div className="grid grid-cols-4 items-center gap-10 px-5">
            <div className="h-[124px] overflow-hidden">
              <img src="/images/blogs/1/1.webp" className="h-[124px] w-full object-cover object-center rounded-xl border-4" />
            </div>
            <div className="col-span-3">
              <h1 className="mb-5 line-clamp-1 text-justify text-2xl font-bold">
                Injury Attorneys and Product Liability: Holding Manufacturers
                Accountable
              </h1>
              <p className="line-clamp-3 text-justify">
                When a product fails and causes harm, it can disrupt your life
                significantly. That's where injury attorneys step in,
                specializing in product liability cases to ensure that
                manufacturers aare held accountable for their negligence. This
                blog explores the crucial role of injury attorneys in these
                legal battles, guiding victims through the complex process of
                securing the compensation they deserve.
              </p>
            </div>
          </div>
        </NavLink>
        <hr />
        <NavLink to="/blogs/2">
          <div className="grid grid-cols-4 items-center gap-10 px-5">
            <div className="h-[124px] overflow-hidden">
              <img src="/images/blogs/2/1.webp" className="h-[124px] w-full object-cover object-center rounded-xl border-4" />
            </div>
            <div className="col-span-3">
              <h1 className="mb-5 line-clamp-1 text-justify text-2xl font-bold">
                How to Handle Insurance Adusters: Strategies Recommended by
                Injury Attorneys
              </h1>
              <p className="line-clamp-3 text-justify">
                The moment following an injury is often fraught with confusion,
                pain, and a whirlwind of decisions to make. One of the most
                pivotal steps in this process involves dealing with insurance
                adjusters-those often unseen yet crucial figures determining the
                fate of your claims. Knowing how to navigate these interactions
                can dramatically influence the compensation you receive, setting
                the stage for a smoother recovery path. Armed with strategies
                recommended by seasoned injury attorneys, this guide aims to
                empower you, offering wisdom to handle insurance adjusters with
                confidence and acumen.
              </p>
            </div>
          </div>
        </NavLink>
        <hr />
        <NavLink to="/blogs/3">
          <div className="grid grid-cols-4 items-center gap-10 px-5">
            <div className="h-[124px] overflow-hidden">
              <img src="/images/blogs/3/1.webp" className="h-[124px] w-full object-cover object-center rounded-xl border-4" />
            </div>
            <div className="col-span-3">
              <h1 className="mb-5 line-clamp-1 text-justify text-2xl font-bold">
                The Impact of Pre-Existing Conditions on Your Injury Claim:
                Legal Perspectives
              </h1>
              <p className="line-clamp-3 text-justify">
                Navigating an injury claim can be a complex and intricate
                process, particularly when pre-existing conditions are involved.
                These conditions can significantly affect the outcome of your
                claim, influencing how insurance companies evaluate and
                compensate your injuries. This guide provides a comprehensive
                look at the legal landscape surrounding pre-existing conditions
                and offers strategic advice for managing their impact on your
                injury claims
              </p>
            </div>
          </div>
        </NavLink>
        <hr />
        <NavLink to="/blogs/4">
          <div className="grid grid-cols-4 items-center gap-10 px-5">
            <div className="h-[124px] overflow-hidden">
              <img src="/images/blogs/4/1.webp" className="h-[124px] w-full object-cover object-center rounded-xl border-4" />
            </div>
            <div className="col-span-3">
              <h1 className="mb-5 line-clamp-1 text-justify text-2xl font-bold">
                7 Red Flags to Watch Out for When Hiring an Injury Attorney
              </h1>
              <p className="line-clamp-3 text-justify">
                Choosing the right personal injury attorney can be the linchpin
                to successfully navigating the complex landscape of legal claims
                and securing the compensation you deserve. However, the path to
                finding a trustworthy advocate is strewn with pitfalls. This
                guide aims to arm you with critical knowledge by highlighting
                seven red flags to watch out for when hiring an injury attorney.
                From spotting under qualified candidates to avoiding those who
                overpromise outcomes, we'll ensure you can make an informed and
                confident decision in your legal representation. Let’s dive into
                these warning signs to help steer clear of potential
                disappointments.
              </p>
            </div>
          </div>
        </NavLink>
        <hr />
        <NavLink to="/blogs/5">
          <div className="grid grid-cols-4 items-center gap-10 px-5">
            <div className="h-[124px] overflow-hidden">
              <img src="/images/blogs/5/1.webp" className="h-[124px] w-full object-cover object-center rounded-xl border-4" />
            </div>
            <div className="col-span-3">
              <h1 className="mb-5 line-clamp-1 text-justify text-2xl font-bold">
                Injury Attorneys and Client Communication: What to Expect
                Throughout Your Case
              </h1>
              <p className="line-clamp-3 text-justify">
                Embarking on a personal injury claim can be intimidating,
                particularly when you're unsure about the process. A key element
                to your case's success is how well your injury attorney
                communicates with you from start to finish. Effective
                communication can demystify the complexities of your case,
                provide comfort during stressful moments, and enhance your
                understanding of the legal proceedings. This article outlines
                what to expect in terms of communication with your injury
                attorney throughout the different phases of your case.
              </p>
            </div>
          </div>
        </NavLink>
        <hr />
        <NavLink to="/blogs/6">
          <div className="grid grid-cols-4 items-center gap-10 px-5">
            <div className="h-[124px] overflow-hidden">
              <img src="/images/blogs/6/1.webp" className="h-[124px] w-full object-cover object-center rounded-xl border-4" />
            </div>
            <div className="col-span-3">
              <h1 className="mb-5 line-clamp-1 text-justify text-2xl font-bold">
                Technology and Personal Injury Law: How Attorneys Utilize
                Innovation for Clients
              </h1>
              <p className="line-clamp-3 text-justify">
                In today’s fast-paced world, technology influences every aspect
                of life, including the legal field. Personal injury law,
                traditionally reliant on paper-based systems and in-person
                interactions, is undergoing a transformation with the
                integration of advanced technologies. From streamlining case
                management to enhancing evidence collection, technology is
                revolutionizing how attorneys manage cases and advocate for
                their clients. This comprehensive exploration delves into the
                ways in which personal injury attorneys are leveraging
                technology to improve outcomes for their clients.
              </p>
            </div>
          </div>
        </NavLink>
        <hr />
        <NavLink to="/blogs/7">
          <div className="grid grid-cols-4 items-center gap-10 px-5">
            <div className="h-[124px] overflow-hidden">
              <img src="/images/blogs/7/1.webp" className="h-[124px] w-full object-cover object-center rounded-xl border-4" />
            </div>
            <div className="col-span-3">
              <h1 className="mb-5 line-clamp-1 text-justify text-2xl font-bold">
                Protecting Your Rights in Medical Malpractice Cases: Guidance
                from Injury Attorneys
              </h1>
              <p className="line-clamp-3 text-justify">
                When you seek medical treatment, you trust that the
                professionals in charge of your care will act responsibly and
                proficiently. However, when that trust is broken due to
                negligence, the consequences can be devastating, leading to
                injuries, worsening conditions, or even death. Navigating the
                complex world of medical malpractice claims requires not only
                understanding your rights but also how to effectively protect
                them. This comprehensive guide, crafted by seasoned injury
                attorneys, offers essential advice for securing justice and
                compensation in medical malpractice cases. This guide, informed
                by expert injury attorneys, provides vital insights into
                navigating these treacherous waters, from initial prevention to
                pursuing a claim.
              </p>
            </div>
          </div>
        </NavLink>
        <hr />
        <NavLink to="/blogs/8">
          <div className="grid grid-cols-4 items-center gap-10 px-5">
            <div className="h-[124px] overflow-hidden">
              <img src="/images/blogs/8/1.webp" className="h-[124px] w-full object-cover object-center rounded-xl border-4" />
            </div>
            <div className="col-span-3">
              <h1 className="mb-5 line-clamp-1 text-justify text-2xl font-bold">
                The Road to Recovery: How Injury Attorneys Assist with
                Rehabilitation and Support
              </h1>
              <p className="line-clamp-3 text-justify">
                Recovering from a serious injury is not only a physical
                challenge but also an emotional and financial ordeal. The role
                of an injury attorney extends beyond the courtroom; they are
                pivotal in guiding clients through the complex process of
                rehabilitation and ensuring that they receive the necessary
                support to restore their quality of life. This article explores
                how injury attorneys are instrumental in assisting clients on
                their road to recovery.
              </p>
            </div>
          </div>
        </NavLink>
      </div>
    </div>
  );
};

export default Blogs;
